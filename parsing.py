...
def get_congressinfo():
    html = "http://apis.data.go.kr/9710000/NationalAssemblyInfoService/getMemberCurrStateList?numOfRows="+str(congress_num)+"&ServiceKey="+api_key
    ...
    #---기본정보뽑는곳
    for k in item:
        deptcd_list.append( k.find("deptcd").get_text())
        name_list.append( k.find("empnm").get_text())
        num_list.append(k.find("num").get_text())
    print("END : 의원기본정보 파싱완료")
    print("START : 의원 상세정보 습득중")
    # ---상세정보 뽑는곳 위에 리스트에서 보고 변수 추가해서 뽑자
    for i in range (0,congress_num):
        url = detail_html+url_deptcd+deptcd_list[i]+url_num+num_list[i]
        ...
        party_list[i] = detailitem.find("polynm").get_text()
        try:
            shrt_list[i] = detailitem.find("shrtnm").get_text()
        except:
            print(url)
def db_insertcongress():
    for i in range(0, congress_num):
        curs.execute(sql, (i+ 1, deptcd_list[i], num_list[i], name_list[i], party_list[i], shrt_list[i]))  # congresstable 채우기
    conn.commit()
